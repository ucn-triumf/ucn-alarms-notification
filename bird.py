...
def call_shifter(level, client, names, numbers):

    name = client.odb_get(f'Shifts/messagebird/{level}')
    delay = client.odb_get(f'Shifts/messagebird/{level}_delay_s')
...
def alarm():
    ...
    # get api token and shifter name from MIDAS
    names = client.odb_get('Shifts/messagebird/Name')
    numbers = client.odb_get('Shifts/messagebird/PhoneNumber')

    for level in ['shifter', 'urc', 'vip']:
        if not call_shifter(level, client, names, numbers):
            return
